<script lang="ts">
  let { notAllowed, gridInput, colorInput, blockedInput, noInference, inputNoInference }: {
    notAllowed?: number[][][],
    gridInput: string,
    colorInput: string,
    blockedInput?: string,
  ...
  const gridNoInf = readGrid(inputNoInference, cell => (cell === "0" ? null : Number(cell)))
    ...
    for (let row = 0; row < 5; row++) {
      for (let col = 0; col < 5; col++) {
        ...
        options[row][col] = options[row][col].filter(
          (option) => gridNoInf[row][col] === null && (grid[row][col] === option || !usedNumbers.has(option)),
        );
      }
    }
  ...
  function getColor(options: number[], row: number, col: number) {
    if (options.length === 0 && gridNoInf[row][col] === null) {
      return 'bg-red-200'
    }
  ...
  <div class="grid grid-cols-[1fr_2fr_2fr_2fr_2fr_2fr] gap-0 p-4">
    {#each grid as row, rowIndex}
      ...
      {#each row as cell, colIndex}
        ...
        {@const availableAllowed = availableOptions.filter(opt => !notAllowedResolved[rowIndex][colIndex].includes(opt))}
        <div
        ...
        >
          ...
          <div class="w-full h-[90%]">
            ...
            >
              ...
              {:else if gridNoInf[rowIndex][colIndex]}
                <span class="font-normal italic">{gridNoInf[rowIndex][colIndex]}</span>
              {:else if availableAllowed.length === 1 && !noInference}
              <span class="font-normal italic">{availableAllowed[0]}</span>
              {/if}
