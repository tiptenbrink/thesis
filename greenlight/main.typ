...
First, note that given an activity $x$ with processing time $duration(x)$ with starting time variable $start(x)$ with domain $[lower(x), upper(x)]$, then for times $t$ s.t. $upper(x) <= t < lower(x) + duration(x)$, we know that $x$ is active. This can by observing that an activity as active at times $start(x) <= t < start(x) + duration(x)$ and then using the bounds. We say that for such $t$, activity $x$ is _mandatory_ (also known as _compulsory_). We also define the _resource profile_: which for each time $t$ is defined as the sum of the usages of all activities that are mandatory at that time. Reasoning about when activities are mandatory, two types of conflict can be identified.  

The fundamental conflict concerns only a single time $t$: Suppose we have a cumulative constraint $C$ with capacity $capacity(C)$ and activities $x, y, z$ in that constraint that are mandatory at some time $t$. Then if the usages $usage(x) + usage(y) + usage(z) > capacity(C)$, there is a conflict.
